...
namespace Schematrix
{
	...
	/// </summary>
	public class ClsUnrar : IDisposable
	{
        static object _deposeLock = new object();
		...
		public ClsUnrar(string archivePathName) : this()
		{
		...
		~ClsUnrar()
		{
            ...
            {
                ClsUnrar.RARCloseArchive(this.archiveHandle);
		...
		public void Close()
		{
			...
			// Close archive
			int result=ClsUnrar.RARCloseArchive(this.archiveHandle);
		...
		public void Open(string archivePathName, OpenMode openMode)
		{
			...
			// Open archive
			handle=ClsUnrar.RAROpenArchiveEx(ref openStruct);
			...
			// Set callback
			ClsUnrar.RARSetCallback(this.archiveHandle, this.callback, this.GetHashCode());
			...
			if(this.password.Length!=0)
				ClsUnrar.RARSetPassword(this.archiveHandle, this.password);
		...
		public bool ReadHeader()
		{
			...
			// Read next entry
			currentFile=null;
			int result=ClsUnrar.RARReadHeaderEx(this.archiveHandle, ref this.header);
		...
		public void Skip()
		{
			int result=ClsUnrar.RARProcessFile(this.archiveHandle, (int)Operation.Skip, string.Empty, string.Empty);
		...
		public void Test()
		{
			int result=ClsUnrar.RARProcessFile(this.archiveHandle, (int)Operation.Test, string.Empty, string.Empty);
		...
		{
			int result=ClsUnrar.RARProcessFile(this.archiveHandle, (int)Operation.Extract, destinationPath, destinationName);
